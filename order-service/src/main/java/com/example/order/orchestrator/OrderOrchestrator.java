	...
	public void handleOrderCreated(OrderCreatedEvent event) {
	 ...
	 @KafkaListener(topics = "inventory-response", groupId = "order-service-group")
	    public void handleInventoryResponse(InventoryResponseEvent event) {
	        logger.info("Received inventory response for order: {}, available: {}", 
...
//	        orderService.handleInventoryResponse(event);
	 	...
	 	private void checkInventory(Order order) {
		    ...
		    List<InventoryCheckEvent.InventoryItem> items = order.getItems().stream()
		        .filter(item -> item != null) 
		        .map(item -> new InventoryCheckEvent.InventoryItem(
		            item.getProductId(), 
		            item.getQuantity()))
	...
	private Order getOrder(OrderCreatedEvent event) {
